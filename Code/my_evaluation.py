...
class my_evaluation:
    ...
    def confusion(self):
        ...
        for label in self.classes_:
            ...
            fp = 0
            fn = 0
            for predicted, actual in zip(self.predictions,self.actuals):
                if (predicted == label) and (actual == label):
                    ...
                    fn += 1
            # tp =  np.sum((self.predictions == label) and (self.actuals == label))
            # fp =  np.sum((self.predictions == label) and (self.actuals != label))
            # tn =  np.sum((self.predictions != label) and (self.actuals != label))
            # fn =  np.sum((self.predictions != label) and (self.actuals == label))
